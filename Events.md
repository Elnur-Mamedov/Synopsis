Вы должны понимать что когда программа создает потоки,
...
Нужно для выполнения одного метода разными потоками в упорядочном виде.
К примеру у вас есть свой `Web Server` в котором вы хотите настроить работу сразу
нескольких клиентов. К примеру однопоточное приложение сможет регать последовательно
1000 пользователей. А в случаи с `Semaphore` вы сможете сделать это в 10 потоках.
по 100 пользователей и это будет в 10 раз быстрее.
...
`SemaphoreSlim` 
Это тоже самое что и `Semaphore`, только он работает с потоками как `Monitor`
То есть он использует потоки нашего процесса и не уведомляет OS о том сколько потоков 
используется в один момент.
